...
const AppSiderbar = () => {
    ...
    const [selectedKey, setSelectedKey] = React.useState<Key>(pathname);
    ...
    <div className="sticky top-[90px] z-10">
      ...
      >
        ...
        {pages.map((page) => (
            <ListboxItem
            key={page.path}
            className={` ${selectedKey.toString().includes(page.path) ? "mt-3 bg-gray-100 dark:bg-gray-800" : ""} `}
            startContent={
            <IconWrapper className={`bg-${getPageIconAndPath(page.path).color}/10 text-${getPageIconAndPath(page.path).color} `}>
                <Icon icon={getPageIconAndPath(page.path).icon} className="text-lg " />
            </IconWrapper>
            }
            textValue=""
            > 
